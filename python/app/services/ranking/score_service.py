...
class ScoreService:
    ...
    MARCAS = [
        "logitech", "redragon", "razer", "hyperx", "corsair", "dazz",
        "fortrek", "multilaser", "xiaomi", "philips", "samsung", "lg",
        "intelbras", "jbl", "apple", "baseus", "anker"
    ]

    @staticmethod
    def _obter_titulo(entidade):
        """Extrai o título de um objeto ou dicionário com segurança."""
    ...
    def calcular(produto, video):
        score = 0
        score += ScoreService.score_similaridade(produto, video)
    ...
    def score_similaridade(produto, video):
        score = 0
        titulo_prod = ScoreService._obter_titulo(produto)
        titulo_vid = ScoreService._obter_titulo(video)

        palavras_produto = set(TitleNormalizer.normalizar(titulo_prod))
        palavras_video = set(TitleNormalizer.normalizar(titulo_vid))

        iguais = palavras_produto & palavras_video

        if palavras_produto:
            score += (len(iguais) / len(palavras_produto)) * 100
    ...
    def score_qualidade(video):
        ...
        elif duracao <= 180: score += 5
        return score

    @staticmethod
    def score_bonus(video):
        score = 0
        titulo = ScoreService._obter_titulo(video).lower()
        bonus = {
            "review": 15, "teste": 10, "unboxing": 10,
            "vale a pena": 10, "comparativo": 8, "original": 5,
            "comprando": 5, "recebido": 5, "funciona": 5
        }
